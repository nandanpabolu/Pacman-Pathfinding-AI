...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # Uncomment one of the following lines to use a specific search strategy
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        # Find the closest dot to eat (= goal)
        distance_food = [(util.manhattanDistance(state, food), food) for food in self.food.asList()]
        if not distance_food:
            return False
        distance, goal = min(distance_food)
        if state == goal:
            return True
        else:
            return False
